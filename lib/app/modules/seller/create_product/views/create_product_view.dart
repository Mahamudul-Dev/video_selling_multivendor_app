  ...
  Widget build(BuildContext context) {
    return Obx(() {
      if (controller.isUploading.value) {
        ...
        return Scaffold(
          appBar:
              AppBar(title: const Text('Publisher Studio'), centerTitle: true),
          body: ListView(
            children: [
              AspectRatio(
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    Obx(() {
                      if (controller.trailerPath.value.isNotEmpty &&
                          controller.trailerThumbnail.value != null) {
                        return AspectRatio(
                          aspectRatio: 16.0/9.0,
                          child: Image(
                            image: FileImage(controller.trailerThumbnail.value!),
                          ),
                        );
                      } else {
                        return AspectRatio(
                    ...
                    Obx(() {
                      if (controller.mainContentPath.value.isNotEmpty &&
                          controller.mainContentThumbnail.value != null) {
                        return AspectRatio(
                          aspectRatio: 16.0/9.0,
                          child: Image(
                            image:
                                FileImage(controller.mainContentThumbnail.value!),
                            fit: BoxFit.cover,
                          ),
                        );
                      } else {
                    ...
                    Obx(() {
                      if (controller.productThumbnail.value != null) {
                        return AspectRatio(
                          aspectRatio: 16.0/9.0,
                          child: Image(
                            image: FileImage(controller.productThumbnail.value!),
                            fit: BoxFit.cover,
                          ),
                        );
                      } else {
                            ...
                            child: Center(
                              child: Column(
                                ...
                                children: [
                                  IconButton(
                                      onPressed: () =>
                                          controller.getProductThumbnail(),
                                      icon: const Icon(Icons.upload)),
                                  Text(
