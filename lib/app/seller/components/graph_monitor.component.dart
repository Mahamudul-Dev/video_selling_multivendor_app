...
class GraphMonitor extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: Padding(
        padding: const EdgeInsets.only(top: 15, left: 22, right: 10, bottom: 10),
        child: Column(
                ...
                ? AspectRatio(
                    ...
                    child: LineChart(
                      LineChartData(
                        ...
                        minY: 0,
                        maxY: salesData!.reduce((value, element) => value > element ? value : element) + 100,
                        lineBarsData: [
                          LineChartBarData(
                            spots: List.generate(
                              salesData?.length ?? 0,
                              (index) => FlSpot(index.toDouble(), salesData?[index] ?? 0.0),
                            ),
                            isCurved: true,
                            colors: [const Color(0xff23b6e6), const Color(0xff02d39a)],
                            barWidth: 4,
                            isStrokeCapRound: true,
                            belowBarData: BarAreaData(show: true, colors: [const Color(0xff23b6e6).withOpacity(0.3), const Color(0xff02d39a).withOpacity(0.3)]),
                          ),
                        ],
                      ),
                    ),
...
SideTitles _buildRightTitles() {
  ...
  if (salesData != null) {
    if (salesData!.isNotEmpty) {
      ...
      double minSale = salesData!.reduce((value, element) => value < element ? value : element);
      double maxSale = salesData!.reduce((value, element) => value > element ? value : element);
      double range = maxSale - minSale;
      double stepSize = range / 5; // You can adjust this value based on your preference
  ...
  return SideTitles(
    showTitles: true,
    getTextStyles: (context, value) => Theme.of(context).textTheme.bodySmall?.copyWith(color: Colors.grey, fontSize: 5),
    getTitles: (value) {
      Logger().i({
  ...
  SideTitles _buildXAxisTitles() {
    return SideTitles(
        showTitles: true,
        getTextStyles: (context, value) => Theme.of(context).textTheme.labelSmall?.copyWith(color: Colors.grey, fontWeight: FontWeight.bold),
        getTitles: (value) {
          Logger().i({
            'Bottom Side Value': value
          });
